...
function edShowButton(button, i, field) {
	if (button.id == 'ed_img') {
		document.write('<input type="button" id="' + button.id + '" class="ed_button" onclick="edInsertImage(\'' + field + '\');" value="' + button.display + '" />');
	}
	else if (button.id == 'ed_link') {
		document.write('<input type="button" id="' + button.id + '" class="ed_button" onclick="edInsertLink(\'' + field + '\', ' + i + ');" value="' + button.display + '" />');
	}
	else {
		document.write('<input type="button" id="' + button.id + '" class="ed_button" onclick="edInsertTag(\'' + field + '\', ' + i + ');" value="' + button.display + '" />');
	}
}
...
function edCloseAllTags(field) {
	var count = edOpenTags.length;
	for (o = 0; o < count; o++) {
		edInsertTag(field, edOpenTags[edOpenTags.length - 1]);
	}
}
...
function edToolbar(field) {
	document.write('<div id="ed_toolbar">');
	for (i = 0; i < edButtons.length; i++) {
		edShowButton(edButtons[i], i, field);
	}
	document.write('<input type="button" id="ed_close" class="ed_button" onclick="edCloseAllTags("'+field+'");" value="close tags" />');
